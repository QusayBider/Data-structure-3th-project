...
void outInOrder(struct Hash array_hash[],
                AVL_T t) {          //this function use to send the word from the Avl tree to hash table
    if (t != NULL) {
...
void insert_to_hash(struct Hash array_hash[], char word[], int freq, int Size) {
    if ((float) Percentage / (float) Size >
        .65) {                           //this line use to check the percentage of full table
        if (is_rehash > 1) {
        ...
        while (array_hash[index].state ==
               'O') {                    //this while use to insert word if it has a collision
...
int hashi(int index, int i,
          int Size) {                     //this function use to get the index in collision of word using is equation
    return (index + i) % Size;
}
